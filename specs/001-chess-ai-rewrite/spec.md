...
- Q: Evaluation function (classical handcrafted vs NNUE pre-trained vs hybrid vs train-our-own) → A: **NNUE evaluation with a public, permissively-licensed, pre-trained network embedded into the binary at build time.** Strength target: 3400–3600 Elo. Search: multi-threaded alpha-beta + iterative deepening + transposition table + SMP. The selected NNUE network MUST be permissively licensed (MIT / Apache-2.0 / CC0 / similar) to avoid GPL infection of the whole executable; Stockfish's GPL-licensed networks are therefore explicitly excluded. Suitable candidates include networks from clean-room Rust NNUE engines such as Velvet, Carp, Black Marlin, or Akimbo. Selecting the specific network and engine codebase is a planning-phase decision.
...
- **Mid-move shutdown**: If the user closes the app while the AI is thinking, the application shuts down cleanly within a small bounded delay (no zombie processes, no crashes). The current game is discarded (persistence is out of scope).
...
- **FR-006a**: The NNUE network weights MUST be **embedded statically into the application binary at build time** (e.g., via Rust's `include_bytes!`) — not loaded from disk at runtime, not downloaded, not trained. The chosen network MUST be released under a permissive open-source license (MIT, Apache-2.0, BSD, CC0, or compatible) so the resulting executable can be redistributed without GPL infection. Stockfish's GPL networks are therefore excluded.
...
- **SC-001 — Strength**: At its maximum configured strength on a modern 8-core CPU at 60 seconds per move, the engine wins or draws at least **95%** of games against a publicly available reference engine that is rated 2500 Elo on a standardized test suite (e.g., 100-game match).
...
- **SC-007 — Offline integrity**: A first-time installation on a machine that has never had network access can complete a full Human-vs-AI game, including hints and AI-vs-AI, with **zero** outbound network attempts and **zero** disk writes outside the user-settings file (verified by packet capture and filesystem audit).
...
- **No GPU is required.** NNUE evaluation is CPU-only (sparse incremental updates + integer SIMD). The CPU thread count is user-configurable; the embedded NNUE network is sized to fit in the single executable and run on CPU within SC-006 (≤ 500 MB RAM, ≤ 75% CPU cores by default).
