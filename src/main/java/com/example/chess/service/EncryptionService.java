...
/**
 * Encryption service for gaze data
...
public class EncryptionService {
    ...
    public void init() throws Exception {
        ...
        logger.info("EncryptionService initialized with AES-256 encryption");
    }

    public PrivacyCompliantDataCollectionService.EncryptedGazeData encrypt(PrivacyCompliantDataCollectionService.AnonymizedGazeData data) {
        try {
            // Convert data to byte array
            String dataString = String.format("%.2f,%.2f,%s,%d,%s", 
                data.gazeX, data.gazeY, data.square, data.timestamp, data.userAction);
            byte[] dataBytes = dataString.getBytes();
            
            // Encrypt data
            byte[] encryptedData = encryptGazeData(dataBytes);
            
            return new PrivacyCompliantDataCollectionService.EncryptedGazeData(encryptedData);
        } catch (Exception e) {
            logger.error("Failed to encrypt gaze data", e);
            return new PrivacyCompliantDataCollectionService.EncryptedGazeData(new byte[0]);
        }
    }
